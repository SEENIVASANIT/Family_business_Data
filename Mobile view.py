...
cur.execute("""CREATE TABLE pgstudent(
			...
			pgsub2 text)
            """)
cur.execute("""CREATE TABLE pgstudent2(
            ID INTEGER PRIMARY KEY AUTOINCREMENT,
	        pgsem1 text,
			pgexam1 text,
			pgsub11 text,
...
<My2>:
    ...
    FloatLayout:
        ...
        MDIconButton:
            icon:'chevron-left'
            user_font_size:'40dp'
            pos_hint: {'center_x':0.9,'center_y':.04}
            #pos_hint: {'center_y':.95}
            theme_text_color:"Custom"
            text_color:'#FF0000'
            on_release:
...
class MY_marks(MDApp):######  main class   ######
    ...
    def open3(self):
        webbrowser.open("https://www.google.com/search?q=google+calculator&rlz=1C1RXQR_enIN958IN958&oq=&aqs=chrome.4.69i59i450l8.396301j0j7&sourceid=chrome&ie=UTF-8")
    def open4(self):
        webbrowser.open("https://translate.google.co.in/?hl=en&tab=rT&sl=ta&tl=en&op=translate")
    ...
    def on_start(self):
        ...
        month1=str(datetime.datetime.now().strftime(''))
        day1 =str(datetime.datetime.now().strftime('%A %d-%m-%Y'))
        self.strng.get_screen('it').date.text=f"{day1} {month1} "
        self.strng.get_screen('it2').date.text=f"{day1} {month1} "
